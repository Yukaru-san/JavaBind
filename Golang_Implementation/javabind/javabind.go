...
// ReceiveMessage returns the next incoming message from java
// Stop by using StopMessageReceiver
func ReceiveMessage() (string, error) {

	// Cant use both receiver simultaniously
	if useAutomaticMessageReceiver {
		return "", &MessageError{ErrorType: 0}
	} else if useManualMessageReceiver {
		return "", &MessageError{ErrorType: 1}
	}
	...
	_, remoteAddr, err := server.ReadFromUDP(msgBuffer)
	...
	// Trim useless null bytes
	msgString := strings.TrimRight(string(msgBuffer), "\x00")

	// Accept connection and / or return message
	if msgString == connectionHello {
		javaClient = remoteAddr

		return fmt.Sprintf("Java connected: %s", remoteAddr.String()), nil
	}

	return msgString, nil
}

// OnMessageReceived catches every received msg and passes it to
// the given func. Stop by using StopMessageReceiver()
func OnMessageReceived(execute func(string)) error {
	...
	// Start loop
	useAutomaticMessageReceiver = true
	for useAutomaticMessageReceiver {
...
// StopMessageReceiver stops currently running UDP reads
func StopMessageReceiver() {
	// Send stop code to the server itself
	server.WriteToUDP(stopAutoReceiveCode, &net.UDPAddr{
		IP:   []byte(serverIP),
		Port: port,
	})

	// Reset receiver informations
	useAutomaticMessageReceiver = false
	useManualMessageReceiver = false
}

// SendMessage sends the given msg to Java
func SendMessage(msg string) error {

	// Fill msg with null bytes if needed
	for len(msg) < 2048 {
		msg += "\x00"
	}

	// send it to the client
	_, err := server.WriteToUDP([]byte(msg), javaClient)
